...
Skill обновляет проверенный локальный snapshot канонического источника `robotyre-stock/v1` и помогает менеджеру искать товары по подтверждённым условиям:

- тип товара;
- сезон, всесезонность, шипы и RunFlat;
...
Ответ отдельно показывает количество SKU и физический остаток, свежесть поколения, предложения поставщиков и неизвестные характеристики. Цена в поиске и выдаче всегда берётся только из `price_sale`; `price_input` не является fallback. Повреждённые или семантически несогласованные поколения не активируются. Предупреждения возвращаются массивом: `source_stale` может сочетаться с причиной fallback, например `network_error`.

Источник `robotyre-stock/v1` не публикует структурированный типоразмер. Параметр `--size` поэтому не поддерживается и безопасно возвращает `query_unsupported`. Skill не извлекает размер из названия товара.
...
Границы машинного контракта и публичной проекции описаны в [references/robotyre-v1-contract.md](references/robotyre-v1-contract.md).
...
Минимальный env-файл содержит точно один URL машинных данных — URL manifest:
...
Не добавляйте в env URL файлов `products.jsonl`, `offers.jsonl` и `archive.zip`: skill берёт их пути из проверенного manifest. Замените только примеры логина и пароля значениями, переданными администратором; не вставляйте реальные секреты в чат.
...
```text
$papa-shin-stock-search Найди летние шины, минимум четыре штуки, цена до 8000
```
...
```text
$papa-shin-stock-search Покажи летние шины: минимум 4 штуки, цена продажи до 8000, доставка до 3 дней
```
...
Обе команды возвращают один JSON-объект. Поиск запускайте после refresh. При `stale_cache` данные доступны из предыдущего проверенного поколения, но ответ обязан явно сообщать о всех `warnings`. При `source_stale` сам источник давно не обновлялся; этот код может идти вместе с причиной fallback.
...
| `--size` | Не поддерживается для `robotyre-stock/v1`; любое непустое значение возвращает `query_unsupported`. |
...
- «Найди летние шины, минимум четыре штуки, до 8 000 за единицу».
- «Покажи предложения по шинам с доставкой не дольше трёх дней».
- «Подбери шины на мой автомобиль» — skill сообщает, что не проверяет совместимость и не умеет фильтровать по размеру; при необходимости уточняет только доступные критерии, например сезон и бюджет.
...
| `query_unsupported` | Уберите `--size` или требование типоразмера: канонический источник не публикует его как структурированное поле. |
| `contract_unsupported` | Источник объявил другую версию `contract` или `schema_version`; нужно совместимое обновление skill, а не ослабление проверок. |
| `manifest_invalid` | Заявленный manifest или JSONL не соответствует `robotyre-stock/v1`. Не показывайте URL или исходную строку; повторите штатный refresh и передайте сопровождающему только код ошибки. |
| `generation_mismatch` | `content_generation_id` строки не совпал с manifest. Такое поколение нельзя использовать; повторите штатный refresh. |
...
| `source_stale` в `warnings` | Машинный источник давно не обновлялся. Отразите в ответе этот код и все остальные warnings; не называйте данные актуальными. |

Legacy-поколение без v1 metadata не используется как fallback, автоматически не мигрируется и сохраняется для явного ручного решения администратора.
