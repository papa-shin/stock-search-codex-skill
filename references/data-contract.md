...
Предложения индексируются по `product_id`, цене, сроку доставки и остатку. Текущие production query plans проверены тестом `test_offer_queries_use_composite_index_without_repeated_offer_scans`: в них нет `SCAN o` и `USE TEMP B-TREE`. Поэтому для этих планов суммарная граница объектов схем `main` и `temp` составляет 16 GiB, не считая файлов активного поколения. Будущий запрос, вводящий SQLite sorter, должен до релиза либо не пройти plan-regression проверку, либо получить отдельный внешний disk budget.
